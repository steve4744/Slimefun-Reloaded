...
public class RitualAnimation implements Runnable {
  ...
  public void run() {
    ...
    this.stage += 1;
    Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Slimefun.getSlimefunReloaded(), this, 8);
  }
  ...
  private void checkPedestal(Block pedestal) {
    ...
    if (item == null || itemLock.remove(item) == null) {
      ...
      pedestal.removeMetadata("item_placed", Slimefun.getSlimefunReloaded());
    }
  }

  private void abort() {
    running = false;
    pedestals.forEach(b -> Slimefun.getAltarinuse().remove(b.getLocation()));

    // This should re-enable altar blocks on craft failure.
    Slimefun.getAltarinuse().remove(altar.getLocation());
